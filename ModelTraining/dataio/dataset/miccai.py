        ...
        for root_dir_path in root_dir_paths:
            for patient_id in os.listdir(root_dir_path):
                ...
                for n_slice in range(self.n_slices):
                    ...
                    if n_slice in self.dataset_info[patient_id]['normal']:
                        class_label = 0

                    elif n_slice in self.dataset_info[patient_id]['abnormal']:
                        class_label = 1

                    else:
                        raise Exception('Slice num {} of PatientID {} does not exist in the dataset_info.'.format(
                            n_slice, patient_id
                        ))

                    series['class_label'] = class_label
