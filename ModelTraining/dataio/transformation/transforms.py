...
class RandomRotate(object):
    ...
    def __call__(self, sample):
        ...
        if 'seg_label' in sample.keys():
            try:
                rotated_label = K.geometry.transform.warp_perspective(label, transform, dsize=(H, W), mode='nearest')
            except TypeError:
                rotated_label = K.geometry.transform.warp_perspective(label, transform, dsize=(H, W), flags='nearest')
...
class NonRandomHorizontalFlip(object):

    def __call__(self, sample: dict):
        image = sample['image']
        apply_flip = sample['apply_flip']
